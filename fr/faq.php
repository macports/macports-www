...
//
// File     : faq.php
// Version  : $Id: faq.php,v 1.1 2002/12/25 17:48:16 kevin Exp $
...
<p>
Une description de Darwinports est plus compréhensible en utilisant une comparaison : c'est une sorte de collection de ports comme ceux de <a href="http://www.freebsd.org/ports">FreeBSD</a>
...
<p>
Même en ne comptant pas les quelques limitations des ports FreeBSD décrites ci-dessus, la "science" de créer des systèmes de construction automatisés est bien plus complexe à première vue qu'il n'y paraît et il y aura toujours des approches nouvelles concernant la façon d'attaquer le problème, ce que nous avons essayé de faire avec Darwinports. Il y a probablement d'autres systèmes, certains d'entre eux ont déjà été mentionnés qui ont essayé à leurs manières de résoudre ce problème et il y aura probablement beaucoup plus de systèmes similaires essayant de trouver une solution unique qui plaise à tout le monde - ça n'est que plus ou moins impossible. Nous invitons toute personne susceptible de juger l'aspect de Darwinports d'après ses propres spécificités et de le considérer comme un projet parallèle plutôt qu'un élan de compétition car il y a plus d'un logiciel qu'aucun autre système sachant gérer permette l'automatisation. 
</p>

<p><strong>Quelles sont les conditions requises pour Darwinports ?</strong></p>

<p>
Il requiert actuellement Mac OS X 10.2 (Jaguar), ayant les Developer Tools d'installés puisque c'est le code de référence que la plupart d'entre nous utilise. Il y également un portage prévu pour permettre une compatibilité avec la 10.1 (Puma) dès que nous aurons pu identifier toutes les conséquences des "variantes" qui ont besoin d'être ajoutées aux différents ports. Le projet Darwinports prend des dispositions pour la version d'OS ou les "variantes" spécifiques d'architecture d'un port et nous souhaitons influencer ce mécanisme pour supporter plusieurs versions d'OS ainsi que plusieurs types d'architecture (pour Darwin/x86) en une manière claire. 
</p>

<p><strong>Est-ce que Darwinports gère également la gestion de paquet ?</strong></p>

<p>
Actuellement, Darwinports construit juste les logiciels depuis les sources, les installe et enregistre le processus d'installation afin de permettre à Darwinports de le désinstaller si vous lui demandiez. Il créera également un binaire "instantané" d'une installation d'un port que vous pourrez donner à quelqu'un d'autre afin de lui permettre d'éviter de repasser par les étapes de construction du port; mais la gestion de paquet est une chose que nous avons délibérément mise de côté pour la "phase II" du projet, nous adopterons probablement alors un système de gestion de paquet déjà existant et feront que Darwinports génère simplement ces paquets à la demande. Même avec une "gestion propre et net des paquet", il sera toujours important de laisser le choix de construire depuis les sources puisque quelque chose doit générer les paquets pour chaque version d'OS ou des différents ports, et les développeurs qui modifient les librairies système ou s'amusant à construire un type de logiciel de différentes manières peuvent trouver insuffisant pour leurs besoins un paquet du binaire déjà "mis en boîte".
</p>

<p><strong>Pourquoi est-ce que Darwinports installe tout dans /opt/local par défaut ?</strong></p>

<p>
Premièrement, cet emplacement peut être modifié par un autre emplacement de votre choix, en éditant /etc/ports/ports.conf donc rien n'est figé. Même l'infrastructure basique de Darwinports, qui s'installe dans /opt/local par défaut peut être installée n'importe où simplement en modifiant la valeur de PREFIX en ligne de commande (reportez au fichier README.fr pour plus de détails). Deuxièmement, nous avons dû sélectionner un CERTAIN emplacement pour que les choses à installer s'installent et qu'ils ne se heurtent pas à des composants ou des choses du système déjà installés dans /usr/local, ainsi nous avons choisi de suivre lâchement la convention de Sun et de choisir /opt/local. 
</p>

<p><strong>OK, donc comment commencer à jouer avec ?</strong></p>

<p>
Reportez-vous à la page web de <a
href="http://www.opendarwin.org/projects/darwinports/fr/">DarwinPorts</a>pour des informations concernant le téléchargement du projet via CVS. Une fois une copie en votre possession, lisez le README situé au premier niveau de la hiérarchie pour l'installation ainsi que les instructions basiques d'utilisation. 
</p>
...
<p>port search ".*"</p>
<p>
port search utilise une expression régulière (regex) comme argument donc vous pouvez chercher un ou des port(s) particulier(s) qui vous intéressent. 
</p>
...
<p>
DarwinPorts ne se construira pas correctement avec les librairies TCL livrées dans les premières versions du paquet TCL de Fink. Mettez à jour votre paquet TCL de Fink, ou assurez-vous que vous utilisez la librairie TCL du système, et reconstruisez Darwinports. 
</p>

<p><i>Norton AntiVirus</i></p>
<p>
Le projet Fink a découvert récemment de nombreux problèmes incluant des kernel panics et des gels durant la mise en place de patchs lorsque certains logiciels d'anti-virus étaient installés. Vous devrez peut-être désactiver tout logiciel anti-virus avant d'utiliser Darwinports ou Fink.
</p>
